...
class Kream:
    ...
    def login(self, htx, data):
        ...
        self.header["X-KREAM-WEB-REQUEST-SECRET"] = htx.split('webRequestSecret:"')[
            -1
        ].split('"')[0]
        self.header["X-KREAM-WEB-BUILD-VERSION"] = htx.split("sentry-release=")[
            -1
        ].split(",")[0]
        self.header["X-KREAM-CLIENT-DATETIME"] = (
            f'{datetime.now().strftime("%Y%m%d%H%M%S%z")}+0900'
        )
        ...
        login_data = {"email": data["id"], "password": data["pw"]}

        htx = httpx.post(
            "https://api.kream.co.kr/api/auth/login",
            headers=self.header,
            data=json.dumps(login_data),
        )
        self.header["Cookie"] = (
            f'{self.header["Cookie"]}; {self.cookie_make(htx.headers)}'
        )
    ...
    def get_sell_list(self):
        ...
        while True:
            htx = httpx.get(
                f"https://api.kream.co.kr/api/o/asks/?cursor={page}&tab=finished&status=all",
                headers=self.header,
            )
            for i in htx.json()["items"]:
                if i["display_type"] == "product_list_info_action":
                    ...
                    write_ws.cell(
                        num + 2,
                        6,
                        int(htxx["price_breakdown"]["processing_fee"]["value"]),
                    )
                    write_ws.cell(
                        num + 2, 7, int(htxx["price_breakdown"]["total_payout"])
                    )
                    ...
                    write_ws.cell(
                        num + 2,
                        11,
                        f'https://kream.co.kr/my/selling/{i["actions"][0]["value"].split("/")[-1]}',
                    )
                    write_wb.save("File/Data.xlsx")
                    print(
                        f'{htxx["product"]["release"]["name"]}|{htxx["product"]["release"]["style_code"]}|{htxx["product_option"]["name"]}|{int(htxx["price_breakdown"]["price"])}|{int(htxx["price_breakdown"]["processing_fee"]["value"])}|{int(htxx["price_breakdown"]["total_payout"])}|{htxx["date_paid"]}'
                    )
